...
def test(cfg):
    ...
    if cfg.LORA_PATH is not None:
        print("Patching Lora into model")
        ...
        model_fine = patch_local(model_fine, cfg)
        
        print("Loading LoRA weights from {}".format(cfg.LORA_PATH))
        
        lora_weights = torch.load(cfg.LORA_PATH, map_location="cpu")["state_dict"]
        model_fine.load_state_dict(lora_weights, strict=False)
...
if __name__ == "__main__":
    ...
    music2genre_ = music2genre(eval(f"cfg.DATASET.{dataname.upper()}.LABEL"))
    music_dir = eval(f"cfg.DATASET.{dataname.upper()}.MUSIC_WAV")
